...
const SDL_PixelFormat = enum(u32) {
    ...
    // Platform-specific aliases (big-endian or little-endian dependent)
    SDL_PIXELFORMAT_RGBA32 = if (std.Target.byteOrder == .big) 0x16462004 else 0x16762004,
    SDL_PIXELFORMAT_ARGB32 = if (std.Target.byteOrder == .big) 0x16362004 else 0x16362004,
    SDL_PIXELFORMAT_BGRA32 = if (std.Target.byteOrder == .big) 0x16862004 else 0x16362004,
    SDL_PIXELFORMAT_ABGR32 = if (std.Target.byteOrder == .big) 0x16762004 else 0x16462004,
    SDL_PIXELFORMAT_RGBX32 = if (std.Target.byteOrder == .big) 0x16462004 else 0x16561804,
    SDL_PIXELFORMAT_XRGB32 = if (std.Target.byteOrder == .big) 0x16161804 else 0x16661804,
    SDL_PIXELFORMAT_BGRX32 = if (std.Target.byteOrder == .big) 0x16661804 else 0x16161804,
    SDL_PIXELFORMAT_XBGR32 = if (std.Target.byteOrder == .big) 0x16561804 else 0x16261804,
};
